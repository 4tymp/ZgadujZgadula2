    ...
    public static void Show(HallOfFame hallOfFame, Difficulty difficulty, Settings settings) {
        ...
        if (top5.Count == 0) {
            ...
            foreach (ScoreEntry entry in top5) {
                string ngPlus = entry.IsNewGamePlus ? " [NG+]" : "";
                Console.WriteLine($"{place}. {entry.PlayerName}{ngPlus} | {settings.CurrentLanguage.Attempts}: {entry.Attempts} | {settings.CurrentLanguage.Time}: {entry.Time.TotalSeconds:F1}s");
                place++;
            }
        }
